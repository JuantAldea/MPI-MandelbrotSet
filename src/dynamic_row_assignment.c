...
void calc_row(window win, int row, int max_iter, uchar * buffer)
{
    ...
    for (i = 0; i < win.pixels_width; i++) {
        ...
        buffer[i] =
            ((mandelbrot_iteration(zr, zi, max_iter) ==
              max_iter) ? 255 : 0);
    }
}
...
void client(window win, int com_root, int com_rank, int max_iter)
{
    ...
    while (1) {
        receive_time +=
            mpi_irecv_time(&communication_buffer, 1, MPI_INT, com_root,
        ...
        send_time +=
            mpi_isend_time(buffer, win.pixels_width, MPI_UNSIGNED_CHAR,
                           com_root, DATA, MPI_COMM_WORLD, &request);
        wait_time += mpi_wait_time(&request, &status);
        tasks_solved++;
    }
...
void server(window win, int com_size, int com_rank, uchar * image)
{
    ...
    uchar *buffer = (uchar *) malloc(sizeof(uchar) * win.pixels_width);
    int *current_task_asignement =
        (int *) malloc(sizeof(int) * win.pixels_width);
    ...
    for (i = 1; i < com_size; i++) {
        if (i < total_tasks) {
            ...
            send_time +=
                mpi_isend_time(&current_task_asignement[i], 1, MPI_INT, i,
    ...
    while (tasks_solved < total_tasks + 1) {
        ...
        recv_time +=
            mpi_irecv_time(buffer, win.pixels_width, MPI_UNSIGNED_CHAR,
                           MPI_ANY_SOURCE, DATA, MPI_COMM_WORLD,
                           &recv_request);
        ...
        if (current_task < total_tasks + 1) {
            ...
            send_time +=
                mpi_isend_time(&current_task_asignement[status.MPI_SOURCE],
                               1, MPI_INT, status.MPI_SOURCE, TASK,
            ...
            send_time +=
                mpi_isend_time(&current_task_asignement[status.MPI_SOURCE],
                               1, MPI_INT, status.MPI_SOURCE, FINISH,
                               MPI_COMM_WORLD, &send_request);
        }

        memcpy(image + previous_task * win.pixels_width, buffer,
               sizeof(uchar) * win.pixels_width);

        if (previous_task != total_tasks && previous_task != 0) {
            int reflexion = win.pixels_height - previous_task;
            memcpy(image + reflexion * win.pixels_width, buffer,
                   sizeof(uchar) * win.pixels_width);
        }
    }
    ...
    double *full_statistics =
        (double *) malloc(sizeof(double) * 6 * com_size);
    ...
    for (i = 0; i < com_size; i++) {
        printf("%d  %f  %f  %f  %f  %d\n", (int) full_statistics[6 * i],
               full_statistics[6 * i + 1], full_statistics[i + 2],
               full_statistics[6 * i + 3], full_statistics[6 * i + 4],
               (int) full_statistics[6 * i + 5]);
    }
    ...
    if (com_rank == com_root) {
        ...
        uchar *image =
            (uchar *) malloc(sizeof(uchar) * win.pixels_height *
                             win.pixels_width);
        memset(image, 0,
               sizeof(uchar) * win.pixels_height * win.pixels_width);
