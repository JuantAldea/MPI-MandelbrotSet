...
void calc_row2(window win, int row, int max_iter, uchar * buffer)
{
    ...
    for (int i = 0; i < win.pixels_width; i++) {
        ...
        buffer[i] =
            (uchar) ((mandelbrot_iteration(zr, zi, max_iter) /
                      (float) max_iter) * 255);
    }
}
             ...
             int task_size)
{
    ...
    uchar *buffer =
        (uchar *) malloc(sizeof(uchar) *
                         (size_t) (win.pixels_width * task_size));
    ...
    while (1) {
        ...
        receive_time +=
            mpi_irecv_time(&assignment, 2, MPI_INT, com_root, MPI_ANY_TAG,
        ...
        for (int i = 0; i < assignment[1]; i++) {
            calc_row2(win, assignment[0] + i, max_iter,
                      &buffer[i * win.pixels_width]);
        }
        ...
        send_time +=
            mpi_isend_time(buffer, win.pixels_width * assignment[1],
                           MPI_UNSIGNED_CHAR, com_root, DATA,
                           MPI_COMM_WORLD, &request);
        wait_time += mpi_wait_time(&request, &status);
    ...
    double statistics[6];
    memset(statistics, 0, sizeof(double) * 6);
    ...
    MPI_Gather(statistics, 6, MPI_DOUBLE, NULL, -1, MPI_DOUBLE, com_root,
               MPI_COMM_WORLD);
    free(buffer);
}

double populate_nodes(task task_assignment[], int com_size)
{
    double time = 0;
    MPI_Request request;
    for (int i = 1; i < com_size; i++) {
        ...
        time +=
            mpi_isend_time(&task_assignment[i], 2, MPI_INT, i,
                           task_assignment[i][1] > 0 ? TASK : FINISH,
                           MPI_COMM_WORLD, &request);
    }
    return time;
}
             ...
             uchar * image, int max_iters)
{
    ...
    uchar *data_buffer =
        (uchar *) malloc(sizeof(uchar) *
                         (size_t) (win.pixels_width * task_size));
    task *current_task_assignment =
        (task *) malloc(sizeof(task) * (size_t) com_size);
    ...
    while (tasks_solved < total_tasks) {
        ...
        recv_time +=
            mpi_irecv_time(data_buffer, win.pixels_width * task_size,
                           MPI_UNSIGNED_CHAR, MPI_ANY_SOURCE, DATA,
        ...
        get_task(current_task_assignment, status.MPI_SOURCE,
                 received_data_task);
        //send new task
        ...
        if (is_last_task(current_task)) {
            ...
            send_time +=
                mpi_isend_time(current_task, 2, MPI_INT, status.MPI_SOURCE,
                               FINISH, MPI_COMM_WORLD, &send_request);
        } else {
            //MPI_Send(current_task, 2, MPI_INT, status.MPI_SOURCE, TASK, MPI_COMM_WORLD);
            send_time +=
                mpi_isend_time(current_task, 2, MPI_INT, status.MPI_SOURCE,
                               TASK, MPI_COMM_WORLD, &send_request);
        }
...
//#define PRINT_IMAGE
#ifdef PRINT_IMAGE
        memcpy(image + received_data_task[0] * win.pixels_width,
               data_buffer,
               sizeof(uchar) * (size_t) (win.pixels_width *
                                         received_data_task[1]));
        for (int i = 0; i < received_data_task[1]; i++) {
            ...
            memcpy(image +
                   (win.pixels_height - received_data_task[0] -
                    i) * win.pixels_width,
                   data_buffer + (i * win.pixels_width),
                   sizeof(uchar) * (size_t) win.pixels_width);
        }
    ...
    statistics[4] = wait_time;
    statistics[5] = (double) tasks_solved;
    double *full_statistics =
        (double *) malloc(sizeof(double) * (size_t) (6 * com_size));
    memset(full_statistics, 0, sizeof(int) * (size_t) (6 * com_size));

    MPI_Gather(statistics, 6, MPI_DOUBLE, full_statistics, 6, MPI_DOUBLE,
               com_rank, MPI_COMM_WORLD);
    char path_statistics[100];
    sprintf(path_statistics, "statistics_%d_%d_%d_%d", win.pixels_width,
            max_iters, task_size, com_size);
    freopen(path_statistics, "w", stdout);
    printf("#  total     send      recv      wait      tasks\n");
    for (int i = 0; i < com_size; i++) {
        for (int j = 0; j < 6; j++) {
            printf("%f ", full_statistics[i * 6 + j]);
        }
        printf("\n");
        //printf("%f  %f  %f  %f  %f  %f\n", full_statistics[6*i], full_statistics[6*i+1], full_statistics[i+2], full_statistics[6*i+3], full_statistics[6*i+4], full_statistics[6*i+5]);
    }
...
void dynamic2(int argc, char *argv[])
{
    ...
    if (com_size < 2) {
        printf("Client-server needs at least two computing nodes\n");
        MPI_Finalize();
        exit(-1);
    }
        ...
        uchar *image =
            (uchar *) malloc(sizeof(uchar) *
                             (size_t) (win.pixels_height *
                                       win.pixels_width));
        memset(image, 128,
               sizeof(uchar) * (size_t) (win.pixels_height *
                                         win.pixels_width));
        server2(win, com_size, com_rank, task_size, image, max_iters);
        char path[100];
