...
uchar *continuous_rows_mandelbrot(window win, int first_row, int n_rows, int max_iter) {
    ...
    uchar *image = (uchar *) malloc(sizeof(uchar) * (size_t)(n_rows * win.pixels_width));
...
void continuous_rows_version(int argc, char *argv[]) {
    ...
    if (com_rank == 0) {
        buffer = (uchar *) malloc(sizeof(uchar) * (size_t)((win.pixels_height + 1) * win.pixels_width));
        statistics = (double *)malloc(sizeof(double) * (size_t)(com_size));
    }
    ...
    t = MPI_Wtime() - t;
    MPI_Gather(&t, 1, MPI_DOUBLE, statistics, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    if (com_rank == 0) {
        ...
        for (i = 1; i < win.pixels_height/2; i++){
            memcpy(buffer  + (size_t)(win.pixels_width * (win.pixels_height - i)), buffer + (size_t)(win.pixels_width * i), (size_t)(win.pixels_width));
        }
        char path[100];
